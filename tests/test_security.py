...
from api.core.security import (
    ...
    hash_secret,
    verify_password,
    verify_password_equal_time,
)
from api.core.security import _decoy_password_hash
...
def test_token_vault_str_round_trip():
    """The shape actually used for a bank account number."""
    token = "0123456789"
...
def test_equal_time_verify_matches_plain_verify_for_a_real_account():
    stored = hash_password("correct horse")
    assert verify_password_equal_time("correct horse", stored)
    assert not verify_password_equal_time("wrong horse", stored)


def test_absent_account_fails_without_short_circuiting():
    """An unknown email must not return faster than a wrong password.

    Timing is too noisy to assert directly, so this pins the mechanism: the
    decoy is a real bcrypt hash at the same cost factor a live password gets,
    so verifying against it costs a full round rather than returning early.
    """
    assert verify_password_equal_time("anything", None) is False

    decoy = _decoy_password_hash()
    live = hash_password("correct horse")
    assert decoy.startswith("$2b$")
    assert decoy.split("$")[2] == live.split("$")[2]  # identical cost factor
    assert _decoy_password_hash() == decoy  # generated once per process


def test_decoy_hash_is_not_a_password_anyone_can_supply():
    """The decoy must never verify, or an unknown email would look valid."""
    for guess in ("", "decoy", "password", "correct horse"):
        assert not verify_password(guess, _decoy_password_hash())
